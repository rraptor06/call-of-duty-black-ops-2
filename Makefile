...
ifeq ($(wildcard .env),)
	...
	go get -u
	go mod tidy
	go build -ldflags "-X 'main.serverBuildString=$(BUILD_STRING)'" -o ./build/call-of-duty-black-ops-2

docker:
	docker build -t call-of-duty-black-ops-2 --build-arg BUILD_STRING="$(BUILD_STRING)" .
